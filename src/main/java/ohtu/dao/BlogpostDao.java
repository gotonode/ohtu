...
/**
 * A DAO object for the Blogposts. This class talks with the ohtu.database and
...
public class BlogpostDao implements ObjectDao<Blogpost, Integer> {
    ...
    public Blogpost findByTitle(String title) throws SQLException, ParseException {
        ...
        PreparedStatement stmt = conn.prepareStatement("SELECT*FROM bookmark,blogpost WHERE bookmark.title=? AND bookmark.id=blogpost.id");
    ...
    public Blogpost findById(Integer id) throws SQLException, ParseException {
        Connection conn = database.getConnection();
        PreparedStatement stmt = conn.prepareStatement("SELECT*FROM bookmark,blogpost WHERE bookmark.id=? AND bookmark.id=blogpost.id");
        stmt.setInt(1, id);
        ResultSet rs = stmt.executeQuery();
        if (rs.next()) {
            Blogpost output = constructBlogpostFromResultSet(rs);
    ...
    private Blogpost constructBlogpostFromResultSet(ResultSet rs) throws SQLException, ParseException {
        ...
        Blogpost blogpost = new Blogpost(rs.getInt("id"), rs.getString("title"), date, rs.getString("author"), rs.getString("url"));
        return blogpost;
    }
