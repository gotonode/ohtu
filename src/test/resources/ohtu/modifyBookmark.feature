Feature: User can modify a bookmark of type blogpost by given id (US4,US13)
  ...
  Scenario: the field of the bookmark which is left empty won't be modified (US4)
    Given two blogposts have been created and saved to the database
    And command modifying is selected
    When existed id 1 is entered to be modified
  ...
  Scenario: bookmark will not be modifed if user gives an id which isn't exist in his bookmark list (US13)
    Given two blogposts have been created and saved to the database
    And command modifying is selected
  ...
  Scenario: system will remind the user of the empty database if user tries to modify something from an empty database (US4)
  ...
  Scenario: Url cannot be updated if the new one is invalid (US4)
    Given two videos have been created and saved to the database
    And command modifying is selected
    When existed id 1 is entered to be modified
